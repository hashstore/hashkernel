...
Cake_REF, CakeRack_REF, QuestionMsg_REF, DataChunkMsg_REF, ResponseMsg_REF = (
    GlobalRef(f'hashkernel.bakery.cake:{n}') for n in (
        "Cake", "CakeRack", "QuestionMsg", "DataChunkMsg", "ResponseMsg"
    )
)
...
class CakeType(SmAttr):
    ...
    def __str__(self):
        return f'mode={self.mode},gref={self.gref},' \
...
class CakeTypes(metaclass=TypesProcessor):
    ...
    JOURNAL = CakeType(
        mode=CakeMode.GUID,
        modifiers=[CakeProperties.IS_JOURNAL])
    FOLDER = CakeType(modifiers=[CakeProperties.IS_FOLDER],
                      gref=CakeRack_REF)
    ...
    JOURNAL_FOLDER = CakeType(
        mode=CakeMode.GUID, gref=CakeRack_REF,
